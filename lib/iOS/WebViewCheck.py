...
class WebViewCheck(Base):
    def scan(self):
        ...
        set_values_for_key(key='IOSWEBVIEWCHECKINFO',
                           zh='检测iOS App程序中是否可利用Webview组件跨域访问读取或修改文件的风险',
        ...
        with open(f'{self.appPath}/ClassDump', 'r') as f:
            ...
            results.extend(
                line.strip()
                for line in lines
                if 'allowFileAccessFromFileURLs' in line
                or 'allowUniversalAccessFromFileURLsActivity' in line
            )
